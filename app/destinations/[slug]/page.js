  ...
  return (
    <div>
      <section className="relative h-[52vh] min-h-[380px]">
        <Image src={destination.image} alt={`${destination.name}, ${destination.country}`} fill sizes="100vw" className="object-cover" priority />
      ...
      <section className="mx-auto max-w-6xl px-6 py-16 grid md:grid-cols-3 gap-12">
        ...
        <div className="h-fit rounded-2xl border border-white/10 bg-steel p-6 sticky top-28">
          <div className="flex justify-between text-sm mb-4 pb-4 border-b border-white/10"><span className="text-slateSoft">Duration</span><span className="font-semibold">{destination.duration}</span></div>
          <div className="flex justify-between text-sm mb-6"><span className="text-slateSoft">Starting from</span><span className="font-semibold text-lg">${destination.price.toLocaleString()}</span></div>
          <button onClick={() => openDrawer(destination)} data-cursor-hover className="w-full rounded-full bg-mint text-arctic text-sm font-bold py-3.5 hover:bg-white transition-colors">Book Experience</button>
        </div>
      </section>
    </div>
  );
