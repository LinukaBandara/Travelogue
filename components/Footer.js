...
export default function Footer() {
  ...
  return (
    <footer id="about" className="border-t border-white/10 bg-steel/40 mt-32">
        ...
        <div>
          ...
          <div className="flex flex-col gap-2.5">
            {destinations.slice(0, 5).map((d) => (
              <Link key={d.slug} href={`/destinations/${d.slug}`} className="text-sm text-slateSoft hover:text-mint transition-colors">
                {d.name}
              </Link>
            ))}
          </div>
          ...
          {subscribed ? (
            ...
            >
              <input
                ...
                className="flex-1 min-w-0 rounded-lg bg-arctic border border-white/10 px-3 py-2.5 text-sm text-white placeholder:text-slateSoft/50 focus:outline-none focus:border-mint"
              />
              <button type="submit" className="rounded-lg bg-mint text-arctic text-sm font-semibold px-4 py-2.5 flex-shrink-0">
                Join
              </button>
            </form>
          )}
